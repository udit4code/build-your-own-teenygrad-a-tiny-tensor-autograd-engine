...
def tensor_mean(x, axis=None, keepdim=False):
    ...
    count_tensor = tensor_from_data(float(count))
    out = Div.apply(summed, count_tensor)
    # Step 5: Remove reduced dimensions if requested
    if not keepdim:
        if axis is None:
            new_shape = ()
        else:
...
def tensor_mean_via_numpy_helpers(x, axis=None, keepdim=False):
...
def tensor_matmul_2d(a, b):
    ...
    # Expand both operands to the full (m, k, n) shape BEFORE multiplying.
    # This keeps the broadcast inside autograd-aware ops, so Mul sees equal
    # shapes and Expand.backward sums the gradients back to (m, k, 1)
    # and (1, k, n) respectively.
...
def sparse_categorical_cross_entropy(logits, labels):
    ...
    correct = tensor_gather(log_probs, labels)
    # Step 3: Mean over the batch.
    loss = tensor_mean(correct)
